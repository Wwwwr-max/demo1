...
- `train.py`
  加载配置和数据，完成训练和验证。`train_T()` 执行一轮训练，`verify()` 在验证集上计算准确率，并在每个 epoch 使用 TensorBoard 记录指标；验证集 `macro-F1` 更高时保存 `best_model.pth`。
  `verify()` 用来计算精确率，召回率，F1
...
本次运行以训练集 3000 条、验证集 1000 条、`batch_size=12`、学习率 `1e-5`、最大训练轮数 14 进行；由于连续 3 轮验证集 `macro-F1` 未提升，实际训练到第 10 轮后触发早停。

| Epoch | train_loss | dev_acc | p_macro | r_macro | f1_macro |
| --- | ---: | ---: | ---: | ---: | ---: |
| 1 | 1.1816 | 0.8470 | 0.6975 | 0.6957 | 0.6947 |
| 2 | 0.4293 | 0.8500 | 0.7317 | 0.7363 | 0.7306 |
| 3 | 0.2424 | 0.8600 | 0.7526 | 0.7491 | 0.7495 |
| 4 | 0.1419 | 0.8610 | 0.7677 | 0.7448 | 0.7536 |
| 5 | 0.0798 | 0.8710 | 0.7670 | 0.7530 | 0.7579 |
| 6 | 0.0405 | 0.8620 | 0.7562 | 0.7584 | 0.7555 |
| 7 | 0.0231 | 0.8760 | 0.7682 | 0.7685 | 0.7676 |
| 8 | 0.0208 | 0.8630 | 0.7587 | 0.7575 | 0.7568 |
| 9 | 0.0139 | 0.8610 | 0.7615 | 0.7568 | 0.7581 |
| 10 | 0.0096 | 0.8550 | 0.7535 | 0.7429 | 0.7455 |

- 最优 `dev_acc`：**0.8760**，出现在 Epoch 7
- 最优 `macro-F1`：**0.7676**，出现在 Epoch 7
- `best_model.pth` 保存第 7 轮模型权重
- 早停 `patience=3`，第 10 轮结束后触发早停，训练提前结束
